...
from Subfunction.Anomaly_detection import  Various_parameters
from Subfunction.take_your_need import find_line_take_cannula_integration  ,piont_take_cannula,find_line_take_cannula
...
def main_kernel():
    ...
    if not os.path.exists('Data/Generated_data/cut_list_acc/'): #判断所在目录下是否有该文件名的文件夹
        os.mkdir('Data/Generated_data/cut_list_acc/')    
    cut_list_nuber = [[2,5],[2,10]]
    cut_list = ['one_to_one','two_to_one','three_to_one','four_to_one','five_to_one']   #'two_to_two','three_to_three'
    for cut in cut_list:
        ...
        for path in tqdm(path_list): 
            ...
            filtered_image,local2 = Various_parameters.anomalydetect(copy.deepcopy(dst_back_gray_all),cut)
            for cut_number in cut_list_nuber:
        ...
        with open('Data/Generated_data/cut_list_acc/'+cut+'.txt', 'w') as file:
