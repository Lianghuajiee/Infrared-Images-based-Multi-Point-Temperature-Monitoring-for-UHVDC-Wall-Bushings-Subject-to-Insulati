...
def get_two_model_acc():                       
    path_list = os.listdir('Data/Raw_data/')   
    if not os.path.exists('Data/Generated_data/two_acc'): #判断所在目录下是否有该文件名的文件夹
        os.mkdir('Data/Generated_data/two_acc')    
    model_list = ['One_svm', 'IsolationForest','EllipticEnvelope','DBSCAN', 'PCA','KMeans','HBOS','Z_Score','LocalOutlierFactor','Autoencoder']
    combinat = list(range(len(model_list)))
    combinations = list(itertools.combinations(combinat, 2))    
    cut_list = [[2,5],[2,10],[1,10],[1,5]]
    for  quiq,numb in enumerate(combinations):
        ...
        for i,path in enumerate(path_list): 
            loaded_data = get_Preconditioning_data(model_list[numb[1]],path)
            data_list_1 = copy.deepcopy(np.array(loaded_data['data_list']))
            file_area_list = copy.deepcopy(np.array(loaded_data['file_area_list']))
            tsne_list = copy.deepcopy(np.array(loaded_data['tsne_list']))
            filtered_list = copy.deepcopy(np.array(loaded_data['filtered_list']))
            Outcome_site = copy.deepcopy(np.array(loaded_data['Outcome_site']))
            
            loaded_data = get_Preconditioning_data(model_list[numb[0]],path)
            data_list_2 = np.array(loaded_data['data_list'])
            data_list1 = copy.deepcopy(data_list_1)
            data_list2 = copy.deepcopy(data_list_2)
        ...
        with open('Data/Generated_data/two_acc/'+model_list[numb[0]]+'_'+model_list[numb[1]]+'.txt', 'w') as file:
