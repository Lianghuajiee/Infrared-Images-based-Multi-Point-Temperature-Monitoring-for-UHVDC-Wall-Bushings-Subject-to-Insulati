...
def save_to_txt(param_dict,name):
    with open('local_argument_'+name+'.txt', 'w') as file:
        for local, params in param_dict.items():
            file.write(f"{local}: {params}\n")
def read_from_txt(name):
    param_dict = {}
    with open('local_argument_'+name+'.txt', 'r') as file:
        for line in file:
...
def cut_image_conter1(Outcome_site,Casing_profile,dst_back_gray_all,dst_back_rgb,image):
    x_con  = int(np.mean(Outcome_site[:,0]))
    y_con =  int(np.mean(Outcome_site[:,1]))
    max_ri = 0 
...
def cut_image_conter(Outcome_site,Casing_profile,dst_back_gray_all,dst_back_rgb,image):
    ...
    result = np.abs(Outcome_site - np.array([x_con, y_con]))
    max_value_x = np.max(Outcome_site[:,0] -x_con)
    max_value_y = np.max(Outcome_site[:,1] -y_con)

    
    piont=[]
    if y_con-max_value_y-10 >=0 :
        piont.append(y_con-max_value_y-10)   
        y_piont = y_con-max_value_y-10
    else:
        piont.append(0)
        y_piont = max(y_con-max_value_y-10,-10)
    
    if y_con+max_value_y+10 < dst_back_gray_all.shape[0] :
        piont.append(y_con+max_value_y+10)   
    else:
        piont.append(dst_back_gray_all.shape[0])
        
        
    if x_con-max_value_x-10 >=0 :
        piont.append(x_con-max_value_x-10)   
        x_piont = x_con-max_value_x-10
    else:
        piont.append(0)
        x_piont = max(x_con-max_value_x-10,-10)
       
    if x_con+max_value_x+10 <= dst_back_gray_all.shape[1] :
        piont.append(x_con+max_value_x+10)   
    else:
        piont.append(dst_back_gray_all.shape[1])    
    

    square_rgb_image = np.zeros((max_value_y*2+20,max_value_x*2+20,3), dtype=np.uint8) 
    square_rgb_image[(max_value_y*2+20-(piont[1]-piont[0]))//2:(max_value_y*2+20-(piont[1]-piont[0]))//2+(piont[1]-piont[0]), (max_value_x*2+20-(piont[3]-piont[2]))//2:(max_value_x*2+20-(piont[3]-piont[2]))//2+(piont[3]-piont[2]),] = dst_back_rgb[piont[0]:piont[1],piont[2]:piont[3],]
    dst_back_rgb = square_rgb_image

    square_rgb_image = np.zeros((max_value_y*2+20,max_value_x*2+20), dtype=np.uint8) 
    square_rgb_image[(max_value_y*2+20-(piont[1]-piont[0]))//2:(max_value_y*2+20-(piont[1]-piont[0]))//2+(piont[1]-piont[0]), (max_value_x*2+20-(piont[3]-piont[2]))//2:(max_value_x*2+20-(piont[3]-piont[2]))//2+(piont[3]-piont[2])] =dst_back_gray_all[piont[0]:piont[1],piont[2]:piont[3]]
    dst_back_gray_all = square_rgb_image
    
    square_rgb_image = np.zeros((max_value_y*2+20,max_value_x*2+20,3), dtype=np.uint8) 
    square_rgb_image[(max_value_y*2+20-(piont[1]-piont[0]))//2:(max_value_y*2+20-(piont[1]-piont[0]))//2+(piont[1]-piont[0]), (max_value_x*2+20-(piont[3]-piont[2]))//2:(max_value_x*2+20-(piont[3]-piont[2]))//2+(piont[3]-piont[2]),] =image[piont[0]:piont[1],piont[2]:piont[3],]
    image = square_rgb_image

    if y_piont <= -10:
        y_piont = -(max_value_y*2+20-(piont[1]-piont[0]))//2

    if x_piont <= -10:
        x_piont = -(max_value_x*2+20-(piont[3]-piont[2]))//2

    Casing_profile = Casing_profile - np.array([x_piont, y_piont])
    Outcome_site = Outcome_site - np.array([x_piont, y_piont])
    return Outcome_site,Casing_profile,dst_back_gray_all,dst_back_rgb ,image
...
def find_line_take_cannula_integration(name,image,img0,multiple):
        ...
        return Outcome_site,smooth_size    
